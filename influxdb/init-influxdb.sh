...
# ----- CUSTOM PART -----
INFLUXDB_ADMIN_PASSWORD="$(cat /run/secrets/influxdb_admin_password 2>/dev/null)"
INFLUXDB_READ_USER_PASSWORD="$(cat /run/secrets/influxdb_read_user_password 2>/dev/null)"
INFLUXDB_WRITE_USER_PASSWORD="$(cat /run/secrets/influxdb_write_user_password 2>/dev/null || echo writer)"
# ----- END CUSTOM PART -----
